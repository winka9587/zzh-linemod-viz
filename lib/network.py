...
#from .pointnet2_utils.pointnet2_modules import PointnetFPModule, PointnetSAModuleMSG
#from .DGCNN import DGCNN, DGCNN_pc
...
class ModifiedResnet2(nn.Module):
...
class PoseNet_trans(nn.Module):
    def __init__(self, num_points, num_obj):
        ...
        self.conv6_r = torch.nn.Conv1d(64, num_obj*4, 1)
    ...
    def forward(self, img, x, model_points, choose, obj):
        ...
        out_rx = self.conv6_r(ry5).view(bs, self.num_obj, 4, self.num_points)
...
#################################################norm#################################################
def conv_bn_block(input, output, kernel_size=1):
    return nn.Sequential(
        nn.Conv1d(input, output, kernel_size),
        #nn.BatchNorm1d(output),
        nn.LeakyReLU(inplace=True)
    )
class PEFeat_mix(nn.Module):
    def __init__(self, num_points):
        super(PEFeat_mix, self).__init__()
        self.combine1 = CB_Layer()
        self.combine2 = CB_Layer()
        self.conv1 = nn.Linear(3, 64)
        self.conv2 = conv_bn_block(64, 128)

        self.e_conv1 = conv_bn_block(32, 64)
        self.e_conv2 = conv_bn_block(64, 128)

        self.conv5 = conv_bn_block(256, 512)
        self.conv6 = conv_bn_block(512, 1024)
...
class PPFeat_mix(nn.Module):
    def __init__(self, num_points):
        super(PPFeat_mix, self).__init__()
        self.combine1 = CB_Layer()
        self.combine2 = CB_Layer()
        self.conv1 = nn.Linear(3, 64)
        self.conv2 = conv_bn_block(64, 128, 1)

        self.m_conv1 = nn.Linear(3, 64)
        self.m_conv2 = conv_bn_block(64, 128)
        ...
        self.num_points = num_points
    def forward(self, x, model_points):
        x = F.leaky_relu(self.conv1(x)).transpose(1,2)
        emb = F.leaky_relu(self.m_conv1(model_points)).transpose(1,2)
        pointfeat_1 = self.combine1(x,emb)

        x = self.conv2(x)
        emb = self.m_conv2(emb)
        pointfeat_2 = self.combine2(x,emb)

        x = self.conv5(pointfeat_2)
        x = self.conv6(x)
...
class P_P_EFeat_mix(nn.Module):
    def __init__(self, num_points):
        super(P_P_EFeat_mix, self).__init__()
        self.combine = CB_Layer()
        self.conv1 = conv_bn_block(1408, 1408, 1)
        self.conv2 = conv_bn_block(1408, 1408, 1)
        self.conv3 = conv_bn_block(2816, 2816, 1)
        self.conv4 = conv_bn_block(5632, 2816, 1)
        self.conv5 = conv_bn_block(2816, 1408, 1)

        self.ap1 = torch.nn.AvgPool1d(num_points)
        self.num_points = num_points
    def forward(self, PEfeat, PPfeat):
        PEfeat = self.conv1(PEfeat)
        PPfeat = self.conv2(PPfeat)

        PPEfeat = self.combine(PEfeat, PPfeat)#c = 2816
        PPEfeat = self.conv3(PPEfeat)

        ap_x = self.ap1(PPEfeat)

        ap_x = ap_x.view(-1, 2816, 1).repeat(1, 1, self.num_points)
        PPEfeat = torch.cat([PPEfeat, ap_x], 1)#2816+2816
        PPEfeat = self.conv4(PPEfeat)
        PPEfeat = self.conv5(PPEfeat)

        return PPEfeat #1480


class PoseNet_trans_mix(nn.Module):
    def __init__(self, num_points, num_obj):
        super(PoseNet_trans_mix, self).__init__()
        self.num_points = num_points
        self.cnn = ModifiedResnet()
        self.feat1 = PEFeat_mix(num_points)
        self.feat2 = PPFeat_mix(num_points)
        self.feat = P_P_EFeat_mix(num_points)

        #self.conv1_r = torch.nn.Conv1d(1408, 640, 1)
        self.conv1_t = torch.nn.Conv1d(1408, 1024, 1)
        ...
        self.conv3_t = torch.nn.Conv1d(512, 256, 1)
        self.conv3_c = torch.nn.Conv1d(512, 256, 1)
    ...
    def forward(self, img, x, model_points, choose, obj):
        ...
        #rx = F.relu(self.conv1_r(ap_x))
        tx = F.leaky_relu(self.conv1_t(ap_x))
        cx = F.leaky_relu(self.conv1_c(ap_x))

        #rx = F.relu(self.conv2_r(rx))
        tx = F.leaky_relu(self.conv2_t(tx))
        cx = F.leaky_relu(self.conv2_c(cx))

        #rx = F.relu(self.conv3_r(rx))
        tx = F.leaky_relu(self.conv3_t(tx))
        cx = F.leaky_relu(self.conv3_c(cx))

        #rx = self.conv4_r(rx).view(bs, self.num_obj, 4, self.num_points)
        tx = F.leaky_relu(self.conv4_t(tx))
        cx = F.leaky_relu(self.conv4_c(cx))
        ...
        ap_w = self.ap(ap_x).repeat(1,1,self.num_points)
        #rx = torch.cat([ap_x, x_trans], 1)
        rx1 = F.leaky_relu(self.conv1_pr(x_trans))#(64*500)R
        #rx1 = self.sa_pr1(rx1.transpose(1,2).contiguous()).transpose(1,2).contiguous()
        rx2 = F.leaky_relu(self.conv2_pr(rx1))#(128*500)R
        rx2 = self.sa_pr2(rx2.transpose(1,2).contiguous()).transpose(1,2).contiguous()
        rx3 = F.leaky_relu(self.conv3_pr(rx2))#(256*500)R
        #rx3 = self.sa_pr3(rx3.transpose(1,2).contiguous()).transpose(1,2).contiguous()
        rx4 = F.leaky_relu(self.conv4_pr(rx3))#(512*500)R
        rx4 = self.sa_pr4(rx4.transpose(1,2).contiguous()).transpose(1,2).contiguous()
        rx5 = F.leaky_relu(self.conv5_pr(rx4)).transpose(1,2).contiguous()#(1024*500)
        #rx5 = self.sa_pr5(rx5)


        ry = torch.cat([rx5,ap_w],1)
        ry1 = F.leaky_relu(self.conv1_r(ry))#(1024*500)
        ry2 = F.leaky_relu(self.conv2_r(ry1))+rx4.transpose(1,2).contiguous()#(512*500)
        ry3 = F.leaky_relu(self.conv3_r(ry2))+rx3.transpose(1,2).contiguous()#(256*500)
        ry4 = F.leaky_relu(self.conv4_r(ry3))+rx2.transpose(1,2).contiguous()#(128*500)
        ry5 = F.leaky_relu(self.conv5_r(ry4))#(64*500)
        out_rx = self.conv6_r(ry5).view(bs, self.num_obj, 4, self.num_points)
        out_rx = torch.index_select(out_rx[b], 0, obj[b])
...
class PoseRefineNetFeat_trans(nn.Module):
    def __init__(self, num_points):
        super(PoseRefineNetFeat_trans, self).__init__()
        self.conv1 = torch.nn.Conv1d(3, 64, 1)
        self.conv2 = torch.nn.Conv1d(64, 128, 1)

        self.mconv1 = torch.nn.Conv1d(3, 64, 1)
        self.mconv2 = torch.nn.Conv1d(64, 128, 1)

        self.e_conv1 = torch.nn.Conv1d(32, 64, 1)
        self.e_conv2 = torch.nn.Conv1d(64, 128, 1)

        self.conv5 = torch.nn.Conv1d(576, 1024, 1)
        #self.conv6 = torch.nn.Conv1d(512, 1024, 1)

        self.ap1 = torch.nn.AvgPool1d(num_points)
        self.num_points = num_points

    def forward(self, x, emb, model):
        x = F.relu(self.conv1(x))
        emb = F.relu(self.e_conv1(emb))
        model = F.relu(self.mconv1(model))

        pointfeat_1 = torch.cat([x, emb, model], dim=1)#192

        x = F.relu(self.conv2(x))
        emb = F.relu(self.e_conv2(emb))
        model = F.relu(self.mconv2(model))

        pointfeat_2 = torch.cat([x, emb, model], dim=1)#384
...
class PoseRefineNet_trans(nn.Module):
    def __init__(self, num_points, num_obj):
        ...
        self.num_points = num_points
        self.feat = PoseRefineNetFeat_trans(num_points)
    ...
    def forward(self, x, emb, model, obj):
        bs = x.size()[0]
        
        x = x.transpose(2, 1).contiguous()
        model = model.transpose(2, 1).contiguous()
        ap_x = self.feat(x, emb, model)
